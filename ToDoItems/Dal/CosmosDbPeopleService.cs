...
{
    public class CosmosDbPeopleService : ICosmosDbPeopleService
    {
        private readonly Container container;
        public CosmosDbPeopleService(CosmosClient cosmosClient, string databaseName, string containerName)
        {
            container = cosmosClient.GetContainer(databaseName, containerName);
        }
        public async Task<IEnumerable<Person>> GetPeopleAsync(string queryString)
        {
            List<Person> people = new(); // IList does not implement IEnumerable, IEnumerable does not have Add!
            var query = container.GetItemQueryIterator<Person>(new QueryDefinition(queryString));
            while (query.HasMoreResults)
            {
                var result = await query.ReadNextAsync();
                people.AddRange(result.ToList()); // this method is not in IEnumerable so we use List!
            }
            return people;
        }
