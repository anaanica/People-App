...
namespace ToDoItems.Dal
{
	public class CosmosDbService : ICosmosDbService
	{
		...
		{
			List<Item> items = new(); // IList does not implement IEnumerable, IEnumerable does not have Add!
			var query = container.GetItemQueryIterator<Item>(new QueryDefinition(queryString));
			while (query.HasMoreResults)
			{
				var result = await query.ReadNextAsync();
				items.AddRange(result.ToList()); // this method is not in IEnumerable so we use List!
			}
			return items;
		}
    }
